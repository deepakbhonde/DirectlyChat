...
function setTimeGreeting() {
  ...
  let greeting = "";
  if (hour >= 5 && hour < 12) {
    greeting = "ðŸŒ… Good Morning! ðŸ˜Š";
  } else if (hour >= 12 && hour < 17) {
    greeting = "â˜€ï¸ Good Afternoon! ðŸ˜Š";
  } else if (hour >= 17 && hour < 21) {
    greeting = "ðŸŒ™ Good Evening! ðŸ˜Š";
  } else {
    greeting = "ðŸŒ™ Good Night! ðŸŒŸ";
  }
  const greetingSpan = document.getElementById("dynamicGreeting");
  if (greetingSpan) greetingSpan.innerText = greeting;
}

// Call it when page loads and update every minute
setTimeGreeting();
